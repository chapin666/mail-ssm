...
function jump(){	
	...
	if(!isNaN(jumpnum)){//判断是否是数字
		$('#pageForm').attr('action','/email/saveMail.html');
		...
		$.ajax({
			...
			success:function(data){
	    		if(data==1){
			    	...
			    	$('#addtagform').attr('action','/tag/doAddTag2.html');
			    	$('#addtagform').submit();
...
function addetag(src){
	if($(src).val()==-2 || $(src).val()==0){
		...
		if(ids.length<1){
			...
			$('#tagid').val($(src).val());
			$('#tagtype').val(3);
			$('#editsaveform').attr('action','/tag/doAddETag2.html');
			$('#editsaveform').submit();
		}
	}
}
...
function deleteetag(id){
	if(id!=null&&id!=''&&id!=undefined){
		$('#tagid').val(id);
		$('#editsaveform').attr('action','/tag/deleteETag2.html');
		$('#editsaveform').submit();
	}
}
...
function changemainmail(id,tag){
	$.ajax({
		...
		data:{id:id,ismain:tag,draftsmail:1},
		dataType: 'html',
		async: false,
		success:function(data){
    		if(data==1){
    			location.href='/email/saveMail.html';
		    	return true;
		    }else{
		    	alert('星际邮件标记失败');
    			return false;
