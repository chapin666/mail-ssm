...
<div class="layout">
		<div style="font-size:14px;border-bottom-width: 2px;border-bottom: 2px solid #EAEAEA;padding-bottom: 5px;">
			<b>设置邮件地址黑名单 </b>| <a href="javascript:tobl();"  style="text-decoration: none;"  target="spam_blacklist"> 返回"反垃圾"设置 </a>
		</div>
		<div style="min-height: 50px;padding-left: 60px;padding-top: 10px; border-bottom-width: 2px;border-bottom: 2px solid #EAEAEA;">
			<form action="./spam_blacklist_list.html?m=add"  method="post" onsubmit="return subFormBl();">
				<input id="txtbl" name="txtbl" size="30" height="25px" type="text"  >&nbsp;
				<input type="submit" class="btn_gray" value=" 添加到黑名单 "  >
			</form>
		</div>
		...
		<ul>
			<li class="title">
				<span class="listspan1">邮件地址</span>
			...
			<c:forEach var="bl" items="${blacklist}" >					   
				...
				<span class="listspan3">
					<a href="./spam_blacklist_list.html?m=del&txtbl=${bl.reportuser}">删除</a></span>
			    </li>
			</c:forEach>
		</ul>
		<c:if test="${not empty blacklist}">
		 <div class="page">
			...
			<span style="float:right;margin-right:20px;">
				...
				<c:if test="${pageNow>1}">
					<a href="./spam_blacklist_list.html?i=${pageNow-1}">上一页 </a>&nbsp; 
				</c:if>
				<c:if test="${pageNumber>pageNow}">
					<a href="./spam_blacklist_list.html?i=${pageNow+1}">下一页</a>
				</c:if>
			</span>	
		  </div>
		 </c:if>
