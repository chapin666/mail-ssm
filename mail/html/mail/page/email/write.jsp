...
<div class="msgBoxDIV" id = "msgsavecg" style="display: none;">
	<span class="errmsg">已保存到草稿箱</span>
</div>

<div class="msgBoxDIV" id = "msgsavecgNull" style="display: none;">
	<span class="errmsg">未发现改动，无需保存</span>
</div>

<div class="msgBoxDIV" id = "msgSending" style="display: none;">
	<span class="errmsg">正在发送中..........</span>
</div>
...
<div class="main">
	<div class="content">
		...
		<div class="mail mailBG">
			<form action="/email/SendMail.html" method="post" id="sendmailform" enctype="multipart/form-data">
			<input type="hidden" name="email.id" id="emailid" value="${email.id}">
			<input type="hidden" name="email.type" id="type">
			<input type="hidden" name="email.ismain" id="ismain" value="0">
			...
			<ul class="theme">
				...
				<li>
					...
					<c:if test="${not empty showstr}">
						<label class="input">
							<input type="text" name="email.toname" id="recivemail" class="input1" value="${showstr}" onkeydown="if(event.keyCode==8||event.which==8||event.charCode==8){delc(1);}" onkeyup="if(event.keyCode==8||event.which==8||event.charCode==8){delc(4);}"/>
							<span id="showstrcheck"></span>
						</label>
						
					</c:if>
					<c:if test="${empty showstr}">
						<label class="input">
							<input type="text" name="email.toname" id="recivemail" class="input1" value="${email.toname}" onkeydown="if(event.keyCode==8||event.which==8||event.charCode==8){delc(1);}" onkeyup="if(event.keyCode==8||event.which==8||event.charCode==8){delc(4);}"/>
							<span id="showstrcheck"></span>
						</label>
					</c:if>
				...
				<input type="hidden" name="filelistsize" id="filelistsize" value="0"/>
				<li>
					<label class="title">&nbsp;</label>
					<label class="input" style="position:relative;">
						<span id="filespan" style="width:80px;height:28px;position:absolute;z-index:99;">
							<input type="file" name="file" id="attach" style="display:block;filter:alpha(opacity=0);-moz-opacity:0;opacity:0;position:absolute;width:80px;height:28px;" size="1"/>
						</span>
						<a id="addfj" style="background:url('/mail/images/gray/ico_fujian.jpg') no-repeat center left;padding-left: 15px;">添加附件</a><span id="showtoolbar"></span>
				...
				<li id="fujianFileShow" style="display:block;height:auto;line-height:22px;padding-left:50px;">
					<s:iterator value="#request.filelist" id="fl">
						...
						<p id="<%=uuid %>">${fl.filename}&nbsp;&nbsp;
							<a href="javascript:;" style="color:#006699;" 
				...
				<li id="fujianFileShow" style="display:none;margin: 0 0 0 50px;;background: none repeat scroll 0 0 #ECECEC;font: 12px Verdana;width: 1410px;">
				...
				<input type="hidden" id="signContent" value="${signContent}">
					...
					<label class="input" style="padding:2px 0;">
						<textarea id="content" name="email.content">
							${email.content}									
							<c:if test="${fn:length(sdefault)> 0}">
								<p id="psignContent"><br/><br/>					
								------------------
								<br/>${signContent}</p>
							</c:if>
						</textarea>
...
<!-- 				<li>
					<input class="txt"/>
					<img id="serachUnitBtn" class="btnSearch"/>
				</li> -->
				
				<li class="input" onclick="seachA();">
				<input type="text" name="" id="mailkey" onclick="seachB();"/>
							...
							<ul>
								...
								<s:iterator value="#request.contactList" id="cal">
									<c:if test="${cal.addr!=sessionScope.user.username}">
										<%tag++;%>
										<li style="padding-left:18px;display:none;" class="lxr mailgroupsmall" lang="lxr_all" id="lxr_${cal.addr}">
										<a value="${cal.name}&lt;${cal.addr}&gt;" title="${cal.addr}" onclick="settomail(this);">
										<c:if test="${fn:length(cal.name)>7}">${fn:substring(cal.name, 0, 7)}...</c:if><c:if test="${fn:length(cal.name)<=7}">${cal.name}</c:if>
										</a>
										</li>
									</c:if>
								</s:iterator>
												...
												<s:iterator value="#request.contactList" id="cal1">
													<c:if test="${cal1.id eq conl.cid}">
														<c:if test="${cal1.addr!=sessionScope.user.username}">
															<%flag++;%>
															<li style="padding-left:18px;display:none;" lang="lxr_${uglss.id}" class="lxr mailgroupsmall" id="lxr_${cal1.addr}">
															<a value="${cal1.name}&lt;${cal1.addr}&gt;" title="${cal1.addr}" onclick="settomail(this);">
																<c:if test="${fn:length(cal1.name)>7}">${fn:substring(cal1.name, 0, 7)}...</c:if><c:if test="${fn:length(cal1.name)<=7}">${cal1.name}</c:if>
															</a>
															</li>
														</c:if>
													</c:if>
											...
											<c:if test="${userl.unid eq unl.id}">
												<c:if test="${userl.username!=sessionScope.user.username}">
													<script type="text/javascript">
														...
														d.add(i,'${unl.id}','<a class="contactclick" id="lxr_${userl.username}@${sessionScope.host}" title="${userl.username}@${sessionScope.host}" value="${userl.name}">'+lenname1+'</a>','','','','','','');
													</script>
												</c:if>
											</c:if>
										</s:iterator>
