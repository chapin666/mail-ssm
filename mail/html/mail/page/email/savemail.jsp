	...
	<div class="cddelete">
		...
		<div class="createlabel_body">
			...
			<dir class="icon_info_b" style="margin: -7px 12px 8px 0px;"></dir>				
				删除后邮件将不可恢复，确定要删除？<br/><br/>
			</div>
		</div>		
		<div class="createlabel_bottom"> 
			<input type="button" class="btn_blue" value="确定" onclick="movesave(6);" />
			<input type="button" class="btn_gray" value="取消" onclick="$('.cddelete').hide()" />
		</div>
	</div>


<form action="/email/saveMail.html" method="post" id="pageForm">
  	<input type="hidden" name="pagenow" id="pagenow" value="${pagenow}">
	<input type="hidden" name="name" value="${name}">
</form>
<div class="main">
	<div class="content">
		<p class="title"><b>草稿箱</b>(共${totalsize}封)</p>
		<div class="button">
			<span><input type="button" value="" name="deletecgBtn" class="deletecgBtn" onclick="movesave(2);"/></span>
			...
			<span>
				...
				<select name="" id="">
					...
					<option value="4" onclick="movesave(4);">　收件箱</option>
					<option value="5" onclick="movesave(5);">　发件箱</option>
					<option value="6" onclick="movesave(6);">　垃圾箱</option>
				</select>
				-->
			</span>
		...
		<div class="mail">
			<ul class="relation" style="padding:0px;">
				...
				-->
				<form action="/email/EditSaveMail.html" method="post" id="editsaveform">
					<input type="hidden" name="pagenow" id="pagenow" value="${pagenow}">
					...
					<s:iterator value="#request.emaillist" id="eml">
						<li>
							<span class="span7"><input type="checkbox" name="id" class="box" lang="${eml.see}" value="${eml.id}" /><input type="checkbox" name="types" class="box1" value="" style="display:none;" /><input type="checkbox" name="states" class="box2" value="${eml.state}" style="display:none;" /><input type="checkbox" name="files" class="box3" value="${eml.file}" style="display:none;" /><input type="checkbox" name="frommails" class="box4" value="${eml.frommail}" style="display:none;" /></span>
							<a>
								<span class="span22" onclick="location.href='/email/MailInfo.html?id=${eml.id}&ftype=0'"><c:if test="${eml.see eq 2}"><label class="ico1"></label></c:if><c:if test="${eml.see eq 1}"><label class="ico2"></label></c:if><c:if test="${not empty eml.file}"><label class="ico3"></label></c:if></span>
								<span class="span5" onclick="location.href='/email/MailInfo.html?id=${eml.id}&ftype=0'"><c:if test="${not empty eml.tomail}">${fn:substring(eml.tomail,0,22)}</c:if><c:if test="${empty eml.tomail and not empty eml.copyto}">${fn:substring(eml.copyto,0,22)}</c:if><c:if test="${empty eml.tomail and empty eml.copyto and not empty eml.bcc}">${fn:substring(eml.bcc,0,22)}</c:if></span>
								<span class="span6">
									<font onclick="location.href='/email/MailInfo.html?id=${eml.id}&ftype=0';return false;">
									...
									<c:if test="${not empty etaglist}">
										<s:iterator value="#request.etaglist" id="etl">
											<c:if test="${etl.eid eq eml.id and etl.type eq 3}">&nbsp;
												<label>${etl.name}
								...
								<span class="span50">${eml.time}<!-- <c:if test="${eml.ismain eq 1}"><label class="main1" style="display:none;" onmousedown="changemainmail('${eml.id}',2);return false;"></label></c:if><c:if test="${eml.ismain!=1}"><label class="main2" style="display:none;" onmousedown="changemainmail('${eml.id}',1);return false;"></label></c:if>--></span>
							</a>
						</li>
					</s:iterator>
				</form>
