...
<script type="text/javascript" src="/mail/js/delmail.js"></script>
<script type="text/javascript" src="/mail/js/move.js"></script>
<title>单位邮箱-已删除</title>
...
<form action="/email/delMail.html" method="post" id="pageForm">
  	<input type="hidden" name="pagenow" id="pagenow" value="${pagenow}">
	<input type="hidden" name="name" value="${name}">
</form>
<div class="main">
	<div class="content">
		<p class="title"><b>已删除</b>(共${totalsize}封)</p>
		<div class="button">
			<span><input type="button" value="" name="cddeleteBtn" class="cddeleteBtn" onclick="editdel(5);"/></span>
			...
			<span>
				...
				<select name="" id="move" onchange="movedel(this);">
					<option value="-1">移动到...</option>
					<option value="1">　收件箱</option>
					<!-- <option value="4" onclick="movedel(4);">　垃圾箱</option> -->
				</select>
		...
		<div class="mail">
			<ul class="relation" style="padding:0px;">
				...
				-->
				<form action="/email/EditDelMail.html" method="post" id="editdelform">
					<input type="hidden" name="pagenow" id="pagenow" value="${pagenow}">
					...
					<s:iterator value="#request.emaillist" id="eml">
						<li>
							...
							<input type="checkbox" name="boxtype" class="box6" value="${eml.boxtype}" style="display:none;" />
							</span>
							<a>
								...
								<c:if test="${fn:length(eml.title)>35}">${fn:substring(eml.title, 0, 35)}...</c:if><c:if test="${fn:length(eml.title)<=35}">${eml.title}</c:if>
									...
									<c:if test="${not empty etaglist}">
										<c:if test="${eml.boxtype eq 2}">
											<input type="hidden" name="ttype" id="tagtype" value="2" />
											<s:iterator value="#request.etaglist" id="etl">
												<c:if test="${etl.eid eq eml.id and etl.type eq 2}">&nbsp;<label>${etl.name}<font onclick="deleteetag('${etl.id}');return false;">×</font></label></c:if>
											</s:iterator>
										</c:if>
										<c:if test="${eml.boxtype eq 1}">
								...
								<span class="span50">${eml.time}<c:if test="${eml.ismain eq 1}"><label class="main1" onmousedown="changemainmail('${eml.id}',0,'${eml.mailid}','${eml.boxtype}');return false;"></label></c:if><c:if test="${eml.ismain!=1}"><label class="main2" onmousedown="changemainmail('${eml.id}',1,'${eml.mailid}','${eml.boxtype}');return false;"></label></c:if></span>
							</a>
						</li>
					</s:iterator>
				</form>
		...
		<div class="button">
			...
			<span>
				...
				<select name="" id="move1" onchange="movedel(this);">
					<option value="-1">移动到...</option>
					<option value="1">　收件箱</option>
					<!-- <option value="4" onclick="movedel(4);">　垃圾箱</option> -->
				</select>
