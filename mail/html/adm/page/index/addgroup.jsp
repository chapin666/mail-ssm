...
<script type="text/javascript" src="/adm/js/addgroup.js"></script>
</head>
<body>
<div class="main">
	<div class="content">
			...
			<div class="con_box">
				...
				<div class="con_body">
					<div class="addbodylist">
						<form action="/groups/doAddGroups.mail" method="post" name="addGroupsForm" id="addGroupsForm">
						<ul> 
							<li>
								<span class="span1">组名：</span>
								<span><input type="text" name="groups.name" id="name" value="" class="inputname" /></span>
														 	...
														 	<s:iterator value="#request.unitlist" id="unl">
													 			<script type="text/javascript">
																	...
																	if(lenn>7){
														 			...
														 			d.add('${unl.id}','${unl.pid}','<input type="checkbox" class="unitdel" value="${unl.id}" lang="${unl.name}" onclick="getUnitMember(this);"> '+lenname+'','#','','','','','');
																</script>
																<s:iterator value="#request.userlist" id="userl">
																	<c:if test="${userl.unid eq unl.id}">
																		<script type="text/javascript">
																			...
																			d.add(i,'${unl.id}','<a class="lxr" lang="unit_${unl.id}" id="lxr_${userl.username}@${sessionScope.host}" onclick="getUserMember(this,\'${userl.id}\');">'+lenname1+'</a>','','','','','','');
																		</script>
																	</c:if>
																</s:iterator>
															</s:iterator>
